...
function calc() {
  ...
  let sex, height, weight, age, ratio; //проверка хранилища и присвоение значений переменным
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(elem => {
      elem.classList.remove(activeClass); //проверка блока gender
  ...
  calcTotal(); //так как нет делегирования, родительский элемент не нужен
  ...
  function getDinamicInformation(selector) {
    const input = document.querySelector(selector); //при клике начинается проверка по id, для сопоставления

    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        // если не число
        input.style.border = '2px solid red'; //    input.style.boxShadow = '0 4px 15px red';
      } else {
        input.style.border = 'none'; //    input.style.boxShadow = '0 4px 15px rgba(0,0,0,.2)';
      }
...
function cards() {
  ...
  class MenuCard {
    ...
    render() {
      const element = document.createElement('div'); //проверка if(). если в rest оператор ничего не передали
...
function forms(formSelector, modalTimer) {
  ...
  function bindpostData(form) {
    form.addEventListener('submit', event => {
      ...
      form.insertAdjacentElement('afterend', statusMessage); //способ отправки с XMLHttpRequest()
      ...
      const formData = new FormData(form); //для JSON данных

      const json = JSON.stringify(Object.fromEntries(formData.entries())); //то же самое но менее современное
      // const obj = {};
      // formData.forEach((value, key) => {
      //     obj[key] = value;
      // });
  ...
  function showThanksModal(message) {
    const prevModalDialog = document.querySelector('.modal__dialog'); //скрытие контента

    prevModalDialog.classList.add('hide'); //открытие модалки

    (0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)('.modal', modalTimer); //создение элемента. все параметры берутся по классам у удаленного элемента
    ...
    document.querySelector('.modal').append(thanksModal); //для возвращения старого вида модального окна
...
function openModal(modalSelector, modalTimer) {
  const modal = document.querySelector(modalSelector); // 1 вариант
  // modal.style.display = 'block';
  // 2 вариант

  modal.classList.add('show');
  modal.classList.remove('hide'); // 3 вариант
  ...
  if (modalTimer) {
    //если есть что-то то будет очистка
...
function closeModal(modalSelector) {
  ...
  modal.classList.remove('show'); // 3 вариант
  // modal.classList.toggle('show');
...
function modal(btnSelector, modalSelector, modalTimer) {
  ...
  const modal = document.querySelector(modalSelector),
        btnModal = document.querySelectorAll(btnSelector); //   closeModalBtn = document.querySelector('[data-close]');
  ...
  window.addEventListener('scroll', showModalEndScrolling); //всплывает когда домотал до конца страницы
}
...
function timer(id, deadLine) {
  ...
  function getTimeRemaining(endtime) {
    const t = Date.parse(endtime) - Date.parse(new Date()) - 3 * 60 * 60 * 1000,
          //можно ставить только new Date,без date.parse
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      const t = getTimeRemaining(endtime); //пезультат работы функции (объект со свойствами, где знач. переменных)
...
document.addEventListener('DOMContentLoaded', () => {
  //44 урок модификация Модального окна
  const modalTimer = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimer), 30000); //всплывает через 3 сек 
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2022-01-01');
